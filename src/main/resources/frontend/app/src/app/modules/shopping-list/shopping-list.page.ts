...
export class ShoppingListPage implements OnInit {
	...
	getCurentShoppingListStatus(shoppingList: ShoppingList) {
	...
	editShoppingListItems(shoppingList: IShoppingList) {
		this.router.navigate(['shopping-list-add-items'], {state: {shoppingList: shoppingList}});
	}


	async addShopingListToFavourites(shoppingList: IShoppingList) {
	...
	markShoppingListsAsCompleted() {
		this.shoppingLists.forEach(shoppingList => {
			let allShoppingListItems = this.getAllShoppingListItems(shoppingList);
			let purchased = this.getPurchasedShoppingListItems(shoppingList);
